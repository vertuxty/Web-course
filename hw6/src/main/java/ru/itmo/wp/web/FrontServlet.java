...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        Method beforeMethod = findMethod(pageClass, route.getBefore());
        Method afterMethod = findMethod(pageClass, route.getAfter());
...
//        putUser(request, view);
    ...
    private Method findMethod(Class<?> pageClass, String nameMethod) {
        Method method = null;
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                method = clazz.getDeclaredMethod(nameMethod, HttpServletRequest.class, Map.class);
